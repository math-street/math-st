...
**Goal:** Determine whether exponent-vector, subset-product, or logarithmic-
lattice methods can find an ordinary polylog-smooth integer in the central
surface interval in polynomial time, then perform the mandatory tenth-session
self-audit.

**Prediction (written before completing the algorithm audit):**
[CONJECTURE] Fixed-prime exponent searches will have only polynomially many
values and insufficient logarithmic resolution, while growing-dimension
subset-product searches will recover density only by introducing an
exponential search or an unproved average-case lattice assumption.

**Did:**
- Proved a counting obstruction for every fixed support bound, even when the
  supporting primes are selected from the growing polylogarithmic factor base
  after the input prime is known.
- Reduced unrestricted exponent vectors to a repeated-weight subset-sum
  instance and tracked the rounding scale forced by the surface interval.
- Checked the original Lagarias--Odlyzko low-density theorem, Bringmann's
  pseudopolynomial algorithm, and Hittmeir's Smooth Subsum Search scope.
- Performed the mandatory tenth-session repository audit.

**Found:**
- [PROVED] Bounded-support smooth products cover only
  $X^{1/4}(\log X)^{O(1)}$ prime centers in a dyadic surface-order range,
  while the prime number theorem supplies $\Theta(\sqrt X/\log X)$ centers.
  Hence infinitely many prime inputs are missed.
- [PROVED] A safe integer rounding of the full logarithmic subset sum needs
  scale $\Omega(mX^{1/4})$ and an exponentially large numerical target.
- [CITED] Bringmann's $\widetilde O(n+t)$ theorem is pseudopolynomial in that
  target, and Lagarias--Odlyzko prove success only for almost all low-density
  instances; the minimally rounded structured instance has density tending
  to infinity.
- [CITED] Smooth Subsum Search is explicitly heuristic and addresses smooth
  factorization values rather than a worst-case target interval.

**Prediction vs. outcome:** [PROVED] Confirmed for the audited generic
routes.  The bounded-support side admits a rigorous coverage obstruction;
the growing-support side exposes exponential precision rather than a checked
polynomial-time theorem.

**Did not work:** [PROVED] Neither pseudopolynomial subset sum nor the
low-density LLL theorem turns the RH existence result into a polynomial-time
finder at interval width $X^{3/4}$.

**Changed my mind about:** [PROVED] The bounded-support obstruction does not
require fixing the actual primes in advance.  It still holds when each input
chooses its best bounded-size support from all primes up to $(\log X)^K$.

**Next:** Audit higher-dimensional selected subgroups and cofactor-projection
encodings, where the dimension-one cardinality obstruction no longer applies;
keep the smooth-order finder and explicit surface seed as independent gaps.
